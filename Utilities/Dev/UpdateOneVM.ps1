...
$tempFile = [System.IO.Path]::GetTempFileName()
$response = Invoke-RestMethod -Uri "http://guhuajun:8000/api/cmdb/devices/virtualmachine/594.json" -Method Put -Body $jsonBody -Headers $headers -ContentType "application/json" -WebSession $webSession -OutFile $tempFile
$response = ConvertFrom-Json -InputObject (Get-Content -Path $tempFile -Encoding UTF8)
$response
Remove-Item -Path $tempFile
